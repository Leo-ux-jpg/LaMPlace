...
def place(
    ...
    place_records = []
    l_flow,lf = get_l_flow(placedb,f'graph_data/l_flow/{placedb.benchmark}.npy')
    m2m_flow = None
    ...
    if best_value != my_inf:
        ...
        for cnt in range(stop_round):
            print(cnt)
            node_a = random.sample(rank_macros(placedb,lf,l_flow)[:5],1)[0]
            node_b = random.sample(candidates, 1)[0]
            swap(place_record[node_a], place_record[node_b], grid_size)
            placed_macros, value = l_mask_placer(
                node_id_ls, placedb, grid_num, grid_size, place_record,l_flow,lf,m2m_flow
            )
            if value >= best_value:
                swap(place_record[node_a], place_record[node_b], grid_size)
            else:
...
def main():
    ...
    result_dir = os.path.join("results_lamplace", f"{benchmark}_{seed1}_{args.name}")
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    if not os.path.exists(os.path.join(result_dir, f"pl")):
        os.makedirs(os.path.join(result_dir, f"pl"))
    ...
    start = time.time()
    place_records = place(
        init_round, stop_round, placedb, grid_num, grid_size, curve_file, placement_file,result_dir
    )
    end = time.time()
    ...
    for i in range(5):
        ...
        pic_file = os.path.join(result_dir, f"{benchmark}_{value:.3f}.png")
        placement_file = os.path.join(result_dir, f"placement_{value:.3f}.csv")
        draw_macros(placedb, placement_file, grid_size, None, pic_file,None)
        pl_file_for_refine = os.path.join(result_dir, f"pl/{benchmark}_{value:.3f}.gp.pl")
